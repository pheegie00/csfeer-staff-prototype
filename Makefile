...
test-e2e: ## Run e2e tests (TEST=path optional)
	...
	@sleep 5
	docker compose exec app uv run pytest $${TEST:-tests/e2e/} -v -m e2e -s --durations=10

test-e2e-ci: ## Run e2e tests in CI (TEST=path optional)
	$(CI_COMPOSE) exec app uv run pytest $${TEST:-tests/e2e/} -v -m e2e -s --screenshot=off --video=off --durations=10
