...
3. **Gather implementation evidence**:
   ...
   if git diff HEAD~N..HEAD --name-only | grep -q "^api/"; then
       cd api && pytest && mypy src/ && ruff check . && cd ..
   fi
   
   # For Infrastructure changes
   if git diff HEAD~N..HEAD --name-only | grep -q "^infrastructure/"; then
       cd infrastructure && terraform validate && terraform fmt -check && cd ..
   fi
